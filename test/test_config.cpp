...
{
protected:
    // Config file under test. Its parent directory deliberately does not exist yet, so
    // get_config has to create it the way it would on a fresh install.
    fs::path config_path;
...
// Saves and restores the environment variables default_path() reads, so these tests
// cannot leak into each other or into the rest of the suite.
...
private:
    ...
    static auto restore(const char* name, const std::optional<std::string>& value) -> void
    {
        if (value.has_value())
        {
