    ...
    -> std::expected<void, ConfigError>
{
    ...
    {
        bot_token = "0";
        std::cerr << "Bot token environment variable was missing please go to "
            << path << " and fill value in\n";
    }
    if (server_id.empty())
    {
        server_id = "0";
        std::cerr << "Server ID environment variable was missing please go to "
            << path << " and fill value in\n";
    }
    std::string tmp =
    ...
    try
    {
        ...
        fs::permissions(path, skim_perms, fs::perm_options::replace);
    }
    catch (const fs::filesystem_error& e)
    {
        return std::unexpected(ConfigError{
            ConfigError::Kind::create_failed,
            std::format("Could not secure config file with only owner read write permissions located at: {}",
                        path.string())
        });
    }
    return {};
